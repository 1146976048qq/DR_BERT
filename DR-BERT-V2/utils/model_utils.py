...
def prepare_pack_padded_sequence_asp(inputs_words, seq_lengths, descending=True):
    """
    for rnn model
    按照句子长度从大到小排序，
    ...
    # 降序文本，降序文本长度，一开始不排序的索引
    return sorted_inputs_words

def function(sentence, aspect):
    """
    重加权函数
    """

    # 重加权函数
    # sentence: [batch_size, seq_len, hidden_size]
    # aspect: [batch_size, hidden_size]
    output = torch.add(sentence, aspect.unsqueeze(1).expand_as(sentence)) # [batch_size, seq_len, hidden_size]
